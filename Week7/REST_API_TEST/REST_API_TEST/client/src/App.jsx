...
import axios from "axios"; // axios 설치해야 통신 가능
import {
  ButtonGroup,
  Container,
  DeleteButton,
...
function App() {
  ...
  const fetchPosts = () => {
    ...
    axios // 백엔드와 통신할 때 쓰는 라이브러리리
      .get("http://localhost:4000/posts") // get 요청
      .then((response) => { 
        setPosts(Array.isArray(response.data) ? response.data : []);
        setError(null);
      })
      /**
       * response == 백엔드에서 오는 요청
       * response.data로 받고 싶은 결과를 가져오고 post에 저장
       */
      .catch((err) => {
